...
### - 2019.05.24
 ...
 그래서 오늘의 작업 계획은,
-   [x] 네이버 개봉예정작 목록 긁는 함수 (아마도 Functions 이용?)
-   [x] 네이버 영화 데이터 긁어 json으로 저장하는 함수 (아마도 Functions 이용?)
-   [x] GCP Coloud 스케줄러 등록

### - 2019.05.27
GCP firebase functions, firestore, cloud 스케줄러를 이용하여 개봉예정작을 긁고 저장하는 것은 끝났다. 이제 노가다가 될지도 모르는 수동 api 영화 코드 매칭 작업을 할 페이지를 만들어야해서 고민중이다. 이참에 관리자 페이지라도 만들어줘야 하는 것인지...

### - 2019.06.05
그렇다. 관리자 페이지를 정말 만들었다. 이전에 회사 일을 할 때 접했던 앤트디자인과 react조합으로 만든 페이지에 개봉예정작을 쭉 뿌리고 리스트에서 영화를 선택하면 우측에 Drawer가 노출되며 감독,영화 이름으로 api에 검색을 하고 버튼을 누르면 해당 영화 코드를 내 firestore에 네이버 코드와 함께 추가한다. 그러고선 회사일이 좀 바빠져서 작업을 정지하고 관리자 페이지에서 api 매칭만 하고 있었는데 아무래도 감독+영화명 조합으로 검색하니 80%의 영화는 잘 검색되어서 부분 자동화를 더 해야겠다. 이번주내로 작업할 수 있으면 좋겠는데 과연 시간이 날지...
